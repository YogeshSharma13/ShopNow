...
const sendMail = async (otpData,name) =>{
  ...
  let info = await transporter.sendMail({
    ...
    text: `Hi ${name}, 
    You recently requested to reset your password for your ShopNow account. ${otpData.code} is the otp to change the password. this is only valid for 10 minutes.
    If you did not request to reset your password, Please ignore this email or contact at sharmayogesh13@outlook.com if you have any questions.
    Thanks,
    ShopNow Team`, // plain text body
...
router.get("/getUserOrders", fetchUser, async (req, res) => {
  try {
    ...
    const orders = await Orders.find({buyer:userId});
    res.json(orders);
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Internal Server Error");
